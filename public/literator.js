...
(function() {
  var ConsoleSegment, ConstraintSegment, DocumentSegment, MarkdownSegment,
  ...
  window.LiterateLoader = (function() {
    function LiterateLoader() {
      ...
      this.print_buffer = [];
      this.segments = [];
      this.segment_types = [ConstraintSegment, CodeSegment, ConsoleSegment, MarkdownSegment];
    }
    ...
    LiterateLoader.prototype.insertNewSegment = function(index, segment_type) {
      var seg;
    ...
    LiterateLoader.prototype.findSegmentIndexBySegment = function(segment) {
      var i, index, _i, _ref;
      index = null;
      for (i = _i = 0, _ref = this.segments.length; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
        if (this.segments[i] === segment) {
          index = i;
        }
      }
      assert(index != null, "Tried to find nonexistant segment.");
      return index;
    };

    LiterateLoader.prototype.destroySegment = function(segment) {
      this.segments.splice(this.findSegmentIndexBySegment(segment), 1);
      return this.render();
    };

    LiterateLoader.prototype.insertAfter = function(segment) {
      var buttons, i, index, me, s, _i, _j, _len, _ref, _ref1,
        _this = this;
      index = null;
      me = this;
      for (i = _i = 0, _ref = this.segments.length; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
      ...
      assert(index != null, "Invalid insertAfter called on LiterateLoader");
      buttons = [];
      _ref1 = this.segment_types;
      for (_j = 0, _len = _ref1.length; _j < _len; _j++) {
        s = _ref1[_j];
        buttons.push({
          text: s.readable_name,
          onclick: (function(segment, i, m) {
            return function() {
        ...
        "Or, go back:": [
          {
            ...
            red: true,
            onclick: function() {
              return overlaymenu.close();
            }
          }
        ]
      };
      return overlaymenu.display();
    };
    ...
    LiterateLoader.prototype.contentUpdated = function() {
      ...
      return $('.constraints').each(function() {
        if ($(this).attr('data-constraints') != null) {
          return $(this).html(hljs.highlight('coffeescript', $(this).attr('data-constraints')).value);
        } else {
          $(this).html(hljs.highlight('coffeescript', $(this).html()).value);
          return console.warn("Note: you have a code element that is relying on HTML -> HTML transfer. This is unreliable.");
        }
  ...
  DocumentSegment = (function() {
    ...
    DocumentSegment.readable_name = "DocumentSegment";
    ...
    DocumentSegment.prototype.insertAfter = function() {
      return lit.insertAfter(this);
    };

    DocumentSegment.prototype.destroy = function() {
      var o,
        _this = this;
      o = new OverlayMenu({
        'Are you sure you want to delete this segment?': [
          {
            text: "Delete it!",
            onclick: function() {
              return lit.destroySegment(_this);
            }
          }, {
            text: "Nevermind.",
            red: true
    ...
    DocumentSegment.prototype.encapsulate = function() {
      var delete_button, edit_button, insert_button, me, s;
      me = this;
      ...
      insert_button.click(function() {
        return me.insertAfter.call(me);
      });
      this.dom_element.append(insert_button);
      delete_button = $('<div class="delete-button">DELETE<div>');
      delete_button.click(function() {
        return me.destroy.call(me);
      });
      this.dom_element.append(delete_button);
  ...
  window.CodeSegment = (function(_super) {
    ...
    CodeSegment.readable_name = "Code";
  ...
  MarkdownSegment = (function(_super) {
    ...
    MarkdownSegment.readable_name = "Markdown";
  ...
  ConstraintSegment = (function(_super) {
    __extends(ConstraintSegment, _super);

    function ConstraintSegment() {
      ConstraintSegment.__super__.constructor.call(this);
      this.allow_editing = true;
    }

    ConstraintSegment.identifier = /^\t\:/;

    ConstraintSegment.readable_name = "Constraint";

    ConstraintSegment.prototype.finish_loading = function() {
      var line;
      this.lines = (function() {
        var _i, _len, _ref, _results;
        _ref = this.lines;
    ...
    ConstraintSegment.prototype.reload_code = function() {
      this.dom_element.removeClass('error');
      this.dom_element.children(".constraints").remove();
      this.dom_element.append(this.render());
      return lit.contentUpdated();
    };

    ConstraintSegment.prototype.run = function() {
      var error, error_element, error_message, js_code;
      this.my_element().removeClass('error');
      this.my_element().find('.error_message').remove();
      js_code = "";
      try {
        return js_code = constraintsolver.execute(this.code);
      } catch (_error) {
        error = _error;
        this.my_element().addClass('error');
    ...
    ConstraintSegment.prototype.load = function(code) {
      var content;
      content = $.trim(code);
      return this.code = content;
    };

    ConstraintSegment.prototype.render = function(self) {
      var element;
      element = $("<div class='constraints'>" + this.code + "</div>");
      element.attr('data-constraints', this.code);
      return element;
    };
  ...
  ConsoleSegment = (function(_super) {
    ...
    ConsoleSegment.readable_name = "Console";
    ...
    ConsoleSegment.prototype.run = function() {
      this.my_element().children('.console').replaceWith(this.render());
      return true;
    };
  ...
  */
  ...
  window.OverlayMenu = (function() {
    function OverlayMenu(options) {
      this.options = options;
      this.dom_element = $('.insert-menu');
    }

    OverlayMenu.prototype.build = function() {
      var button, button_element, menu, section, _i, _len, _ref, _results,
        _this = this;
      ...
      for (section in _ref) {
        menu = _ref[section];
        this.dom_element.append("<h2>" + section + "</h2>");
        for (_i = 0, _len = menu.length; _i < _len; _i++) {
          ...
          this.dom_element.append(button_element);
        }
        _results.push(this.dom_element.append("<br style='clear:both' />"));
      }
      return _results;
    };
    ...
    OverlayMenu.prototype.display = function() {
      var _this = this;
      this.build();
      $('.codeblanket').show();
      $('.insert-menu').show();
      return $('.codeblanket').unbind('click').click(function() {
        return _this.close();
      });
    };
