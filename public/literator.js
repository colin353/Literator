...
(function() {
  ...
  window.LiterateLoader = (function() {
    function LiterateLoader() {
      ...
      this.print_buffer = [];
      this.segments = [];
      this.segment_types = [CodeSegment, ConsoleSegment, MarkdownSegment];
    }
    ...
    LiterateLoader.prototype.request_metamorphasis = function(segment, type_string) {
      var data, i, new_type, s, _i, _j, _len, _ref, _ref1, _results;
      ...
      _results = [];
      for (i = _j = 1, _ref1 = this.segments.length; 1 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 1 <= _ref1 ? ++_j : --_j) {
        if (this.segments[i] === segment) {
          data = this.segments[i]["export"]();
          this.segments[i] = new new_type();
          this.segments[i].load(data);
          this.segments[i].edit();
          break;
        } else {
  ...
  DocumentSegment = (function() {
    ...
    DocumentSegment.type_title = "Base DocumentSegment";

    DocumentSegment.can_convert = true;
  ...
  window.CodeSegment = (function(_super) {
    ...
    CodeSegment.type_title = "CoffeeScript";
    ...
    CodeSegment.prototype.insertAfter = function() {
      return alert("inserting...");
    };
    ...
    CodeSegment.prototype.edit = function() {
      ...
      $('.CodeMirror').find('.insert-button').unbind('click').click(function() {
        return me.insertAfter.call(me);
      });
      $('.CodeMirror').find('.type-selector').unbind('change').change(function() {
        var new_type;

        new_type = $(this).val();
        if (confirm("Save and change type to " + new_type + "?")) {
          me.finish_editing.call(me);
          return lit.request_metamorphasis(me, new_type);
        }
      });
      $('.CodeMirror').show();
      $('.codeblanket').show();
  ...
  MarkdownSegment = (function(_super) {
    ...
    MarkdownSegment.type_title = "Markdown";
  ...
  ConsoleSegment = (function(_super) {
    ...
    ConsoleSegment.can_convert = false;
  ...
  $(function() {
    var convertible_segment_list, s, _i, _len, _ref;
